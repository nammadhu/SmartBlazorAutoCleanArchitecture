...
S8.add to cache of town
    ...
    private async Task<BaseResult<iCardDto>> CreateCardWithData(CU_CardCommand createCommand, UserDetailBase operatorOnGraphDb, CancellationToken cancellationToken)
    {
        ...
        if (createCommand != null)
        {
            ...
            var toInsertCard = requestCard.CloneBySerializing();
            if (toInsertCard == null) throw new Exception("toInsert Clone() Failed");
        ...
        return new SharedResponse.Wrappers.Error(ErrorCode.NotHaveAnyChangeInData, "CreateCardWithData Failed as Null Request");
    }
}