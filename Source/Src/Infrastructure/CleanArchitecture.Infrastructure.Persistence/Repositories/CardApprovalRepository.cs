...
: GenericRepository<MyTown.Domain.CardApproval>, ICardApprovalRepository
{
    public CardApprovalRepository(DbContextProvider dbContextProvider) : base(dbContextProvider: dbContextProvider)
    {
        ...
        dbCardApproval = dbContextProvider.DbContext.Set<MyTown.Domain.CardApproval>();
    }

    private readonly DbContextProvider dbContextProvider;

    //private readonly DbSet<Town2VerifiedCard> dbTown2VerifiedCard;
    private readonly DbSet<MyTown.Domain.CardApproval> dbCardApproval;
    ...
    public async Task<bool> SetApproverCardOfDraftCard(ApprovalCardSetRequestCommand request, CancellationToken cancellationToken)
    {
        ...
        && x.IdTown == request.IdTown, cancellationToken)))
        {
            ...
            var created = await dbCardApproval.AddAsync(new MyTown.Domain.CardApproval() { IdCard = request.IdCard, IdCardOfApprover = request.IdApproverCard, IdTown = request.IdTown, CreatedBy = request.IdRequestor }, cancellationToken);
    ...
    /// </summary>
    public async Task<List<MyTown.Domain.CardApproval>> GetCardApprovalsOfApprover(List<int> verifiedCardIdsForApproval, CancellationToken cancellationToken, bool? isVerified = null)
    {
    ...
    public async Task<List<MyTown.Domain.CardApproval>> GetCardApprovals(int cardId, CancellationToken cancellationToken, bool? isVerified = null)
    {
