...
public class Card_DraftChangesRepository : GenericRepository<Card_DraftChanges>, ICard_DraftChangesRepository
{
    public Card_DraftChangesRepository(DbContextProvider dbContextProvider) : base(dbContextProvider)
    {
    }
...
public class Card_Repository : GenericRepository<Card>, ICardRepository
{
    public Card_Repository(DbContextProvider dbContextProvider, IMapper mapper, ICardApprovalRepository townCardApprovalRepository, IIdentityRepository accountServices, IAuthenticatedUserService authenticatedUserService, IUserDetailRepository userDetailRepository,
    IAzImageStorage azImageStorage) : base(dbContextProvider: dbContextProvider)
    {
        this.dbContextProvider = dbContextProvider;
        this.mapper = mapper;
        this.townCardApprovalRepository = townCardApprovalRepository;
        this.accountServices = accountServices;
        this.authenticatedUserService = authenticatedUserService;
        this.azImageStorage = azImageStorage;
        this.userDetailRepository = userDetailRepository;

        //dbTown2VerifiedCard = dbContextProvider.DbContext.Set<Town2VerifiedCard>();
        dbCard = dbContextProvider.DbContext.Set<Card>();
        dbCardDraft = dbContextProvider.DbContext.Set<Card_DraftChanges>();
        dbContext = dbContextProvider.DbContext;
    }

    private DbContextProvider dbContextProvider;
    private IMapper mapper;
    private ICardApprovalRepository townCardApprovalRepository;
    private IIdentityRepository accountServices;
    private IAuthenticatedUserService authenticatedUserService;
    private IAzImageStorage azImageStorage;
    ...
    private readonly DbSet<Card> dbCard;
    private readonly DbSet<Card_DraftChanges> dbCardDraft;
    ...
    public async Task<(bool approvedResult, int townIdRefreshRequired)> ApproveCardAsync(ApproveCardCommand request, CancellationToken cancellationToken)
    {
        ...
        var userRoles = await accountServices.GetUserRolesAsync(authenticatedUserService.UserId, cancellationToken);//here cancellation token is possible to
        ...
        if (cardDraft != null)
        {
            ...
            if (userRoles.Intersect(CONSTANTS.ROLES.Approvers).Any())
            {
                ...
                if (userMatchingVerifierCards != null && userMatchingVerifierCards.Any(c => c.Id > 0))
                {
                    ...
                    {
                        newCardApproval = new MyTown.Domain.CardApproval()
                        {
