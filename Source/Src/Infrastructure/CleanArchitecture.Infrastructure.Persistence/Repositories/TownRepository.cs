...
public class TownRepository(DbContextProvider dbContextProvider, IMapper mapper) : GenericRepository<Town>(dbContextProvider), ITownRepository
{
    ...
    public async Task<PagedResponse<TownDto>> GetPagedListAsync(int pageNumber, int pageSize, string name, CancellationToken cancellationToken)
    {
        ...
        return await Paged(
            query.Select(p => mapper.Map<TownDto>(p)),// dont use this p.To<Towns, TownDto>()),
            pageNumber,
